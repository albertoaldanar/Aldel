...
function ConversationsList(props) {
        ...
        function renderMessage(item){
            if(item.tipo_ultimo_mensaje === "text"){
                if(item.desbloqueado === 0){
                    return <Text> <Icon name ="lock" size = {15} color= "gray"/> Mensaje bloqueado</Text>
                } else {
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity style = {styles.chatList} onPress= {() => props.navigation.push("ConversationClient", {data: item})}>
                <View style = {{flexDirection: "row"}}>
                    ...
                    <View>
                        <View style = {{flexDirection: "row"}}>
                            <Text style = {{fontWeight: "600", fontSize: 17}}>{item.consultor.nombre} {item.consultor.apellido_paterno} </Text>

                            {
                                item.estatus_verificacion === 2 ?
                                    <Image source = {VerifiedIcon} style = {{width: 25, height: 25, marginLeft: 5, marginTop: -2}}/>
                                : null
                            }
                        </View>
                ...
                <View>
                    ...
                    {
                        item.ultimo_mensaje.mensajes_sin_leer_cliente > 0 ?
                            <View style = {{borderRadius: 50, backgroundColor: "#03b388", marginTop: 10, padding: 5}}>
                                <Text style = {{color: "white", fontSize: 12, textAlign: "center"}}>
                                    {item.ultimo_mensaje.mensajes_sin_leer_cliente}
                                </Text>
                            </View>
                        : null
                    }
