...
export default class VoiceCall extends Component<Props, State> {
  ...
  saveFirestore(){
    ...
    this.props.saveCallInFirestore(finalDuration, finalPrice, "Phonecall");
    this.endCall();
  }
...

        {
          this.state.peerIds.length > 0 ?   
            <View
              style={styles.max}
            >
              <Text style = {{marginTop: '40%', textAlign: 'center', fontWeight: '300', fontSize: 17}}> {this.props.data.consultor.nombre } {this.props.data.consultor.apellido_paterno }</Text>

              <FastImage
                style = {{width: 150, height: 150, borderRadius: 50, alignSelf: 'center', marginTop: 20}}
                source = {{
                  uri: this.props.data.consultor.foto_perfil,
